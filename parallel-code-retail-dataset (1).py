...
def split_and_count():
    split_csv("retail_dataset.csv",".","splitted",500)
...
with recursionlimit(15000):
    ...
    def get_ordered_itemset(file_name):
        ...
        #     transactions.append(list(data.iloc[row_no]))
            # for item in data.iloc[row_no]:
    ...
    def new_get_ordered_itemset(file_name):
        # cd = split_and_count()
        data = pd.read_csv(file_name, dtype=str)
        data.dropna(inplace=True,axis=1,how='all')
    ...
    ois = get_ordered_itemset(filename)
